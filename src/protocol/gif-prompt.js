/**
 * Nova-aligned GIF sprite-sheet prompt (adapted from ref-nova-image-studio gif-prompt).
 * Host builds this; client may still encode GIF locally from the grid image.
 */
...
/**
 * @param {number} refImageCount
 */
function buildChineseRefIntro(refImageCount) {
  const templatePart =
...
/**
 * @param {{ userPrompt: string, refImageCount?: number, loop?: boolean, closedLoop?: boolean }} input
 */
export function buildGifPrompt(input) {
  const cleanedUser = String(input?.userPrompt || '').trim()
  const refImageCount = Math.max(0, Number(input?.refImageCount) || 0)
  const hasRefs = refImageCount > 0
  const chineseIntro = buildChineseRefIntro(refImageCount)
...
/** Nova GIF grid constants (names only; no secrets). */
export const GIF_GRID = Object.freeze({
  cols: 4,
  rows: 3,
  frameCount: 12,
  aspectRatio: '4:3',
