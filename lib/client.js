...
function cardCaption(it) {
  ...
  const prompt = String(it.prompt || it.name || "").replace(/\s+/g, " ").trim();
  const safePrompt = prompt && !/^(media\/|file:\/\/|\/)/i.test(prompt) && !/\.(png|jpe?g|webp|gif|mp4)$/i.test(prompt) ? prompt : "";
  const snippet = safePrompt ? `${safePrompt.slice(0, 28)}${safePrompt.length > 28 ? "\u2026" : ""}` : "";
  const time = formatCaptionTime(it.createdAt);
  const modeHint = it.kind === "video" ? "\u89C6\u9891" : it.mode ? String(it.mode) : "\u6587\u751F\u56FE";
  if (model && snippet) return `${model} \xB7 ${snippet}`;
  if (model) return `${model} \xB7 ${modeHint}`;
...
function mountGalleryPage(host, opts) {
  ...
  const buildCardHtml = (it, src, isVideo) => {
    ...
    const snip = caption.includes(" \xB7 ") && modelRaw ? caption.slice(caption.indexOf(" \xB7 ") + 3) : caption;
    const metaInner = modelRaw ? `<span data-ws-gallery-card-model title="${escapeHtml5(String(it.model || ""))}">${escapeHtml5(modelRaw)}</span><span data-ws-gallery-card-snip title="${escapeHtml5(caption)}">${escapeHtml5(snip)}</span>` : `<span data-ws-gallery-card-snip title="${escapeHtml5(caption)}">${escapeHtml5(caption)}</span>`;
...
function mountSidebarEntry(opts) {
  ...
  const clearHostSessionChrome = (scope) => {
    ...
    for (const el of nodes) {
      ...
      const cls = String(el.className || "");
      const label = (el.textContent || "").replace(/\s+/g, " ").trim();
      const looksNewSession = el.getAttribute("data-dsh-part") === "new-session" || /newSession/i.test(cls) || /^(New Session|新会话|\+\s*新会话)$/i.test(label);
