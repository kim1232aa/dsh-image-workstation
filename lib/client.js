...
var init_labels = __esm({
  "src/ui/labels.js"() {
    ...
    RESULT_PRIMARY_ACTIONS = Object.freeze(["\u4E0B\u8F7D", "\u91CD\u65B0\u751F\u6210", "\u590D\u5236\u63D0\u793A\u8BCD"]);
...
function mountGalleryPage(host, opts) {
  ...
  const reload = async () => {
    ...
    else setStatus(`\u5DF2\u8BFB ${state.items.length} \u9879 \xB7 ${seatHint}`);
  };
...
var GALLERY_PAGE, IMAGE_PAGE3, VIDEO_PAGE2, CANVAS_PAGE2, STORAGE_KEY, TAGS_KEY, LAYOUT_KEY, HISTORY_KEY_BASE, CTA_RPC_CHANNEL, CTA_RPC_STORAGE_PATHS, CTA_RPC_STORAGE_LIST, CTA_RPC_GALLERY_ADD, CTA_RPC_GALLERY_LIST, CTA_RPC_GALLERY_TAGS, DEFAULT_PATHS, EMPTY_HINT, FILTER_ALL;
...
var VIDEO_CFG_HINT = "\u8BF7\u5230 \u8BBE\u7F6E \u2192 \u63D2\u4EF6 \u2192 dsh-image-workstation \u586B\u5199\u89C6\u9891 Base URL \u4E0E API Key";
...
<div data-ws-page="video" data-ws-video-cols role="region" aria-label="${VIDEO_PAGE}">
  ...
  <section data-ws-col="studio" data-ws-video-studio style="flex:1;padding:0;overflow:hidden;display:flex;flex-direction:column;min-width:0;background:${T2.layer2};">
...
var ADD_NODE_HINT = "\u53CC\u51FB\u7A7A\u767D\u6216\u70B9\u300C\u6DFB\u52A0\u300D\u5EFA\u8282\u70B9\uFF1B\u9009\u914D\u7F6E\u540E\u5E95\u90E8\u300C\u5F00\u59CB\u751F\u6210\u300D\u771F\u51FA\u56FE";
...
<div data-ws-page="canvas" role="region" aria-label="${CANVAS_PAGE}">
  ...
  <div data-ws-canvas-generator ${state.generatorOpen ? "data-open" : ""} aria-label="\u5E95\u90E8\u751F\u6210\u5668">
    ...
    <div>
      ...
      <textarea data-ws-canvas-gen-prompt rows="2" placeholder="\u5199\u63D0\u793A\u8BCD\u6216\u8FDE\u6587\u672C\u8282\u70B9\u540E\u70B9\u5F00\u59CB\u751F\u6210" style="width:100%;min-height:52px;padding:6px 8px;border:1px solid ${T2.border2};border-radius:8px;background:${T2.input};color:${T2.fg};font:inherit;font-size:12px;"></textarea>
    </div>
    <div style="display:flex;flex-wrap:wrap;gap:8px 12px;align-items:center;">
      <div style="display:flex;align-items:center;gap:6px;">
        <span style="${css2.paramLabel}">${PARAM_LABELS.model}</span>
        <input data-ws-canvas-param-model value="${escapeHtml2(state.nodes.find((n) => n.type === "genConfig")?.modelId || DEFAULT_CANVAS_MODEL)}" placeholder="${escapeHtml2(DEFAULT_CANVAS_MODEL)}" style="padding:0 10px;height:28px;border-radius:14px;width:10rem;border:1px solid ${T2.border2};background:${T2.input};color:${T2.fg};font:inherit;font-size:12px;" />
      </div>
      <div style="display:flex;align-items:center;gap:6px;">
    ...
    <button type="button" data-ws-canvas-send style="${css2.cta}">${CTA}</button>
    <p data-ws-canvas-status class="note">\u62D6\u8282\u70B9 \xB7 \u7AEF\u53E3\u8FDE\u7EBF \xB7 ${CTA} \u2192 \u771F /dsh-ws/generate\uFF08\u5931\u8D25\u5982\u5B9E\u62A5\u9519\uFF09</p>
  </div>
</div>
`;
}
function mountCanvasPage(host, opts) {
  ...
  const syncGenerator = () => {
    ...
    if (selected) {
      ...
      if (!String(selected.prompt || "").trim()) {
        const texts = upstreamResourceNodes(state, selected.id).filter((n) => n.type === "text").map((n) => String(n.text || "").trim()).filter(Boolean);
    ...
    nodesEl.innerHTML = state.nodes.map((n) => {
      ...
      if (n.type === "text") {
        ...
        if (src) {
          ...
          body = `<div data-ws-canvas-img-stub data-empty>\u7A7A\u8282\u70B9 \xB7 \u62D6\u5165 / \u7C98\u8D34\u56FE\u7247<br/>\u6216\u70B9\u300C${CTA}\u300D\u51FA\u56FE</div>`;
        }
  ...
  const syncCanvasCtaEnabled = () => {
    ...
    const linked = cfg ? upstreamResourceNodes(state, cfg.id).filter((n) => n.type === "text").map((n) => String(n.text || "").trim()).filter(Boolean) : [];
    ...
    try {
      document.dispatchEvent(
        new CustomEvent("dsh-ws-top-tab", {
          bubbles: true,
          detail: { tab: name2, studioPage: name2 === IMAGE_PAGE }
        })
      );
    } catch (_) {
    }
    ...
    frame.innerHTML = `
      ...
      <div data-ws-cols data-ws-page="image">
        ...
        <aside data-ws-col="history" style="width:${state.paneWidths.history}px;flex-shrink:0;border-right:1px solid ${T.border2};padding:8px;overflow:auto;background:${T.sidebar};display:flex;flex-direction:column;gap:6px;">
          ...
          <div style="font-size:10.5px;color:${T.fg3};line-height:1.3;">\u672C\u673A\u751F\u56FE \xB7 \u5F02\u4E8E\u5DE6\u4FA7\u4F1A\u8BDD</div>
...
function WorkstationSettingsCard(props) {
  ...
  const pull = useCallback(() => {
    ...
    if (snap?.status === "ready" && snap.value) {
      ...
      const visionKeySet = visionSecretMeta === true || visionSecretMeta?.set === true || typeof visionSecretMeta === "object" && visionSecretMeta != null && "set" in visionSecretMeta && visionSecretMeta.set;
