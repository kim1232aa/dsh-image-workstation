...
function cardCaption(it) {
  ...
  const model = modelRaw ? modelRaw.replace(/^.*\//, "").replace(/[-_]/g, " ").slice(0, 18) : "";
  ...
  if (model && time) return `${model} \xB7 ${time}`;
  if (model) return model;
  if (snippet && time) return `${snippet} \xB7 ${time}`;
  if (snippet) return snippet;
  if (it.mode && time) return `${it.mode} \xB7 ${time}`;
  if (it.mode) return String(it.mode);
  return time ? `\u7D20\u6750 \xB7 ${time}` : "\u7D20\u6750";
}
...
function mountGalleryPage(host, opts) {
  ...
  const paintGrid = () => {
    ...
    grid.innerHTML = items.map((it) => {
      ...
      const media = isVideo ? `<video src="${escapeHtml5(src)}" muted playsinline preload="metadata"></video>` : `<img src="${escapeHtml5(src)}" alt="" loading="lazy" decoding="async" />`;
      ...
      if (el instanceof HTMLImageElement) {
        const checkUiAspect = () => {
          ...
          if (hit) {
            ...
            const ar = w / h;
            const commonUi = w === 1280 && h === 720 || w === 1440 && h === 900 || w === 1920 && h === 1080 || w === 1440 && h === 960 || w === 1280 && h === 800 || w === 1366 && h === 768;
    ...
    for (const el of nodes) {
      ...
      try {
        ...
        if (/sessionRow|SessionRow|sessionItem|SessionItem/i.test(cls) && /selected|Selected|active|Active/i.test(cls)) {
          const next = String(el.className || "").split(/\s+/).filter((t) => t && !/selected|Selected|active|Active|current|Current/i.test(t)).join(" ");
