...
function resolveMainContentPane() {
  const direct = document.querySelector('[data-pane="main"]') || document.querySelector('[data-pane="content"]') || document.querySelector('[class*="mainCol"]') || document.querySelector('[class*="contentCol"]') || document.querySelector('[class*="mainPane"]') || document.querySelector('[class*="contentPane"]') || document.querySelector('[class*="workspaceMain"]') || document.querySelector("main");
  ...
  const named = siblings.find(
    (el) => el instanceof HTMLElement && (el.getAttribute("data-pane") === "main" || el.getAttribute("data-pane") === "content" || /main|content|workspace|session|conversation/i.test(String(el.className || "")))
  );
  ...
  const onProbe = async () => {
    ...
    try {
      ...
      if (result?.ok) {
        ...
        setStatus(
          `\u68C0\u6D4B\u5931\u8D25\uFF1A${result?.error?.message || "unknown"}${result?.error?.code ? `\uFF08${result.error.code}\uFF09` : ""}`
        );
      }
    } catch (e) {
