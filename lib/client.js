...
__export(gallery_host_exports, {
  ...
  formatCaptionTime: () => formatCaptionTime,
  galleryHostStyles: () => galleryHostStyles,
  imageElLooksLikeLightUiChrome: () => imageElLooksLikeLightUiChrome,
  itemHasDisplayableThumb: () => itemHasDisplayableThumb,
  itemNeedsHydration: () => itemNeedsHydration,
  looksLikeUiScreenshot: () => looksLikeUiScreenshot,
  ...
  recoverDiskMeta: () => recoverDiskMeta,
...
function looksLikeUiScreenshot(it) {
  if (!it || typeof it !== "object") return false;
  ...
  const pathBlob = [it.relativePath, it.localPath, it.url, it.name].filter(Boolean).map((x) => String(x)).join("\n");
  if (/docs\/ui\/ref|ui\/ref|smoke[-_]?ui|crit[-_]?|gate[-_]?(studio|final)|reinstall-smoke|dsh-[\w.-]*-light|[-_]light\.(png|jpe?g|webp)|preview/i.test(
    pathBlob
  )) {
    return true;
  }
  ...
  if (w && h) {
    ...
    const commonUi = w === 1280 && h === 720 || w === 1440 && h === 900 || w === 1920 && h === 1080 || w === 1440 && h === 960 || w === 1280 && h === 800 || w === 1366 && h === 768 || w === 1536 && h === 864 || w === 1600 && h === 900;
...
function imageElLooksLikeLightUiChrome(img) {
...
function formatCaptionTime(ts) {
  ...
  try {
    ...
    if (diff >= 0 && diff < 45e3) return "\u521A\u521A";
    if (diff >= 0 && diff < 36e5) return `${Math.max(1, Math.floor(diff / 6e4))}\u5206\u949F\u524D`;
    if (diff >= 0 && diff < 864e5) return `${Math.max(1, Math.floor(diff / 36e5))}\u5C0F\u65F6\u524D`;
...
function truncatePrompt(s, max = 16) {
    ...
    for (const key of keys) {
      ...
      for (const entry of parsed) {
        ...
        results.forEach((r, i) => {
          ...
          const relativePath = r?.relativePath ? String(r.relativePath) : base ? `media/generated/${base}` : "";
          const prompt = pickPrompt(r) || entryPrompt || void 0;
          const model = pickModel(r) || entryModel || void 0;
          out.push({
            id: `${id || "hist"}-${i}`,
            url: url || "",
            ...
            mode: snap.mode ? String(snap.mode) : value.mode ? String(value.mode) : void 0,
            model: model || void 0,
    ...
    if (listRes?.ok && listRes.value) {
      ...
      for (const it of rows) {
        ...
        diskItems.push({
          ...
          prompt: pickPrompt(it) || void 0,
          text: pickPrompt(it) || void 0,
          mode: it.mode ? String(it.mode) : void 0,
          model: pickModel(it) || void 0,
          modelId: pickModel(it) || void 0,
    ...
    if (listRes?.ok && listRes.value && typeof listRes.value === "object") {
      ...
      for (const it of rows) {
        ...
        diskItems.push({
          ...
          text: pickPrompt(it) || void 0,
          mode: it.mode ? String(it.mode) : void 0,
          model: pickModel(it) || inferSeatModel(it) || void 0,
          modelId: pickModel(it) || inferSeatModel(it) || void 0,
          ratio: it.ratio ? String(it.ratio) : void 0,
          tagIds: sanitizeTags(it.tagIds || it.tags),
          createdAt: Number(it.createdAt) || Date.now(),
...
async function recoverDiskMeta(items, opts = {}) {
...
function collectLocalMediaItems(diskItems = []) {
  ...
  const pickDisplay = (a, b) => {
    ...
    const prompt = pickPrompt(a) || pickPrompt(b) || void 0;
    const model = pickModel(a) || pickModel(b) || inferSeatModel(a) || inferSeatModel(b) || void 0;
...
function mountGalleryPage(host, opts) {
  ...
  const probeImageSrc = (src) => new Promise((resolve) => {
  ...
  const buildCardHtml = (it, src, isVideo) => {
    ...
    const media = isVideo ? `<video src="${escapeHtml5(src)}" muted playsinline preload="metadata"></video>` : `<img src="${escapeHtml5(src)}" alt="" decoding="async" />`;
    ...
    const snip = promptSnip || (caption.includes(" \xB7 ") ? caption.slice(caption.indexOf(" \xB7 ") + 3) : "");
    const metaInner = modelRaw ? snip ? `<span data-ws-gallery-card-model title="${escapeHtml5(String(modelFull || ""))}">${escapeHtml5(modelRaw)}</span><span data-ws-gallery-card-snip title="${escapeHtml5(caption)}">${escapeHtml5(snip)}</span>` : `<span data-ws-gallery-card-model title="${escapeHtml5(String(modelFull || ""))}">${escapeHtml5(modelRaw)}</span>` : `<span data-ws-gallery-card-snip title="${escapeHtml5(caption)}">${escapeHtml5(caption)}</span>`;
  ...
  const reload = async () => {
    ...
    paintViewSort();
    const grid = page.querySelector("[data-ws-gallery-grid]");
    const n = grid instanceof HTMLElement ? grid.querySelectorAll("[data-ws-gallery-card]").length : 0;
    setStatus(friendlyGalleryStatus(n));
  };
  ...
  const setPage = (tab) => {
    ...
    if (name2 === GALLERY_PAGE || name2 === IMAGE_PAGE3 || name2 === VIDEO_PAGE2 || name2 === CANVAS_PAGE2) {
      try {
        document.documentElement.setAttribute("data-dsh-ws-studio-open", "");
        document.body?.setAttribute("data-dsh-ws-studio-open", "");
        document.documentElement.setAttribute("data-dsh-ws-top-page", name2);
        document.body?.setAttribute("data-dsh-ws-top-page", name2);
    ...
    if (name2 === GALLERY_PAGE) void reload();
  };
  reload();
  return {
    state,
...
function mountSidebarEntry(opts) {
  ...
  const readTopPage = () => document.documentElement.getAttribute("data-dsh-ws-top-page") || document.body?.getAttribute("data-dsh-ws-top-page") || document.querySelector("[data-dsh-ws-studio-host]")?.getAttribute("data-ws-top-page") || "";
  ...
  const stripSelectedTokens = (el) => {
    ...
    if (!cls) return;
    const next = cls.split(/\s+/).filter((t) => t && !/selected|Selected|active|Active|current|Current|isActive|IsActive/i.test(t)).join(" ");
  ...
  const clearHostSessionChrome = (scope) => {
    ...
    for (const el of nodes) {
      ...
      const cls = String(el.className || "");
      const label = (el.textContent || "").replace(/\s+/g, " ").trim();
      const looksNewSession = el.getAttribute("data-dsh-part") === "new-session" || /newSession/i.test(cls) || /^(New Session|新会话|\+\s*新会话)(\b|$)/i.test(label);
      const looksSession = looksNewSession || /session|Session|workspace|Workspace|conversation|Conversation/i.test(cls) || /session|Session|workspace|New Session|新会话/i.test(label);
      ...
      try {
        ...
        if (/sessionRow|SessionRow|sessionItem|SessionItem|session|Session|newSession/i.test(cls) || looksNewSession || /selected|Selected|active|Active|current|Current/i.test(cls)) {
...
function createStudioHost(opts = {}) {
  ...
  const setTopTab = (tab) => {
    ...
    try {
      ...
      const base = "\u751F\u56FE\u5DE5\u4F5C\u53F0";
      const nextTitle = name2 === GALLERY_PAGE ? `\u753B\u5ECA \xB7 ${base}` : `${name2} \xB7 ${base}`;
  ...
  const mountHistoryItem = (jobId, entry, opts2 = {}) => {
    ...
    const line = timeBit ? `${snippet} \xB7 ${modelShort} \xB7 ${timeBit}` : `${snippet} \xB7 ${ratio}`;
    const modelLine = `${modelId} \xB7 ${mode}${ratio ? ` \xB7 ${ratio}` : ""}`;
    const results = Array.isArray(value?.results) ? value.results : [];
      ...
      h(
        ...
        h(
          ...
          h("input", {
            ...
            placeholder: videoKeyConfigured ? "\u5DF2\u914D\u7F6E\uFF08\u7559\u7A7A\u4FDD\u7559\u5BBF\u4E3B\u5BC6\u94A5\uFF09" : "\u7C98\u8D34\u5BC6\u94A5\u540E Save",
        ...
        h(
          ...
          h("input", {
            ...
            placeholder: visionKeyConfigured ? "\u5DF2\u914D\u7F6E\uFF08\u7559\u7A7A\u4FDD\u7559\u5BBF\u4E3B\u5BC6\u94A5\uFF09" : "\u7C98\u8D34\u5BC6\u94A5\u540E Save",
