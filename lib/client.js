...
var INSPIRE_EMPTY_HELPER = "\u672C\u5730\u6848\u4F8B\u672A\u63A5\u5165\uFF0C\u751F\u6210\u540E\u53EF\u4ECE\u7ED3\u679C\u52A0\u5165\u7075\u611F";
...
var css = {
  ...
  chip: (on) => `padding:2px 8px;border:1px solid ${on ? T.border4 : T.border2};border-radius:999px;background:${on ? T.active : "transparent"};color:${on ? T.fg : T.fg2};cursor:pointer;font:inherit;font-size:11.5px;line-height:1.2;`,
  ...
  topTab: (on) => `padding:6px 11px;border:0;background:${on ? T.active : "transparent"};color:${on ? T.fg : T.fg2};cursor:pointer;border-radius:7px;font:inherit;font-size:12.5px;`
};
var HOST_STYLES = `
    ...
    if (resultsEl) {
      ...
      if (!results.length) {
        ...
        for (const r of results) {
          ...
          card.style.cssText = `border:1px solid ${T.border2};border-radius:10px;padding:4px;background:${T.module};max-width:min(48%,280px);overflow:hidden;`;
          if (src) {
            const img = document.createElement("img");
            img.src = src;
            img.alt = "\u751F\u6210\u7ED3\u679C";
  ...
  const ensure = () => {
    ...
    frame.innerHTML = `
      <header data-ws-top-bar>
        ${TOP_TABS.map(
      (t, i) => `<button type="button" data-ws-top="${t}" style="${css.topTab(i === 0)}">${t}</button>`
    ).join("")}
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h("span", { style: { fontWeight: 650, fontSize: 13 } }, "Image workstation \xB7 \u751F\u56FE\u5DE5\u4F5C\u53F0"),
        h(
          "span",
          {
            style: {
