...
function createStudioHost() {
  ...
  const ensure = () => {
    ...
    cta?.addEventListener("click", () => {
      ...
      setStatus(
        `\u5DF2\u89E6\u53D1\u300C${CTA}\u300D\uFF08${skillNote}\uFF1B\u63D0\u793A\u8BCD ${state.prompt.length} \u5B57\uFF1B\u8D1F\u9762\u8BCD ${state.negativePrompt.length} \u5B57\uFF1B${state.ratio}/${state.clarity}/\xD7${state.count}\uFF09\u2014 \u5DF2\u53D1 dsh-ws-generate\uFF08host-proxy \u4FA7 generate \u5DF2\u63A5\u7EBF\uFF09`
      );
      host.dispatchEvent(
