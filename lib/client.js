...
function createStudioHost() {
  ...
  const chipButtonsHtml = (param, values, selected) => values.map((v) => {
    const val = String(v);
    const on = val === String(selected);
    return `<button type="button" data-ws-param="${param}" data-value="${escapeHtml3(val)}" aria-current="${on ? "true" : "false"}" style="${css.chip(on)}">${escapeHtml3(val)}</button>`;
  }).join("");
