...
var css = {
  ...
  topTab: (on) => `padding:3px 9px;border:0;background:${on ? T.active : "transparent"};color:${on ? T.fg : T.fg3};cursor:pointer;border-radius:6px;font:inherit;font-size:11.5px;font-weight:${on ? 600 : 500};line-height:1.2;`
};
var HOST_STYLES = `
...
function createStudioHost() {
  ...
  const ensure = () => {
    ...
    const compressImageDataUrl = (dataUrl, maxEdge = 1280, quality = 0.82) => new Promise((resolve) => {
      try {
        const img = new Image();
        img.onload = () => {
          try {
    ...
    const addRefFromFile = (file) => {
      ...
      reader.onload = async () => {
        const raw = typeof reader.result === "string" ? reader.result : url;
