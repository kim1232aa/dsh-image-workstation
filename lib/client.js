...
__export(client_exports, {
  ...
  CTA_RPC_STORAGE_PATHS: () => CTA_RPC_STORAGE_PATHS,
...
var TOOL_MORE = "\u66F4\u591A";
var TOOL_ENTRIES = Object.freeze(["GIF", "UI \u8BBE\u8BA1"]);
...
function createStudioHost(opts = {}) {
  ...
  const setTopTab = (tab) => {
    ...
    host?.setAttribute("data-ws-top-page", name2);
    if (name2 === VIDEO_PAGE || name2 === IMAGE_PAGE || name2 === CANVAS_PAGE) {
  ...
  const chipButtonsHtml = (param, values, selected) => values.map((v) => {
    const val = String(v);
    const on = val === String(selected);
    return `<button type="button" data-ws-param="${param}" data-value="${escapeHtml5(val)}" aria-current="${on ? "true" : "false"}" style="${css.chip(on)}">${escapeHtml5(val)}</button>`;
  }).join("");
  ...
  const ensure = () => {
    ...
    host.querySelector('[data-ws-action="enhance"]')?.addEventListener("click", async () => {
      ...
      try {
        ...
        setStatus(/unknown|not.?wired|not.?configured/i.test(msg) ? `\u300C${PROMPT_ACTIONS.enhance}\u300D\u672A\u63A5\u7EBF` : `${PROMPT_ACTIONS.enhance}\u5931\u8D25\uFF1A${msg}`);
      }
    });
    ...
    const initialTop = state.topTab === VIDEO_PAGE ? VIDEO_PAGE : state.topTab === CANVAS_PAGE ? CANVAS_PAGE : IMAGE_PAGE;
